        ...
        public static IEnumerable<IModel> ExtractModels(IChunk mainChunk, IPalette palette)
        {
            ...
            foreach (var keyValuePair in transformNodesThatHaveAShapeNode)
            {
                ...
                foreach (int id in ids)
                {
                    ...
                    VectorData3 size = sizeChunks[id].Size;
                    VectorData3 position = GetGlobalTranslation(transformNodeChunk);
            ...
            VectorData3 GetGlobalTranslation(ITransformNodeChunk target)
            {
                VectorData3 position = target.Frames[0].Translation;
